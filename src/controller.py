...
from numpy import zeros, ones, eye, dot, asarray, matmul, hstack, transpose, array, diag
...
def build_optimal_controller(kane, constants, equilibrium_dict):
    ...
    f_A_lin = linear_state_matrix.subs(
        constants).subs(equilibrium_dict)
    f_B_lin = linear_input_matrix.subs(
        constants).subs(equilibrium_dict)
    m_mat = kane.mass_matrix_full.subs(constants).subs(equilibrium_dict)

    A = array(m_mat.inv() * f_A_lin).astype(float)
    B = array(m_mat.inv() * f_B_lin).astype(float)

    Q = diag((10,1,0.01,0.1))

    R = eye(2)

    S = solve_continuous_are(A, B, Q, R)

    K = matmul(matmul(inv(R), transpose(B)), S)
    ...
    C = hstack((eye((f_B_lin.shape[1])), zeros(
